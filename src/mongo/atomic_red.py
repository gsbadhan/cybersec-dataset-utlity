...
class AtomicRed:
    ...
    def create_bulk_record(self, database, collection_name="atomic_red",  delete_collection=False):
        atomic_red_events = config.OUTPUT.get("atomic_red_events")
        ...
        records = self.read_json(file_path=atomic_red_events)
        ...
        # indexes
        collection.create_index('tactic_id', unique=False)
        collection.create_index('sub_technique_id', unique=False)
        

if __name__ == "__main__":
    atomic = AtomicRed()
    db=atomic.get_or_create_database(db_name="cybersec")
    atomic.create_bulk_record(database=db, delete_collection=True)